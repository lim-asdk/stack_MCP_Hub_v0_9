    ...
    try:
        ...
        try:
            with open(config_path, 'r', encoding='utf-8') as f:
                apis = json.load(f)
            finnhub_key = apis.get("finnhub", {}).get("api_key")
        except Exception:
            finnhub_key = None

        if not finnhub_key:
            return {"ok": False, "error": "Finnhub API key not found in config"}
        ...
        url = f"https://finnhub.io/api/v1/company-news?symbol={symbol}&from={from_date.strftime('%Y-%m-%d')}&to={to_date.strftime('%Y-%m-%d')}&token={finnhub_key}"
